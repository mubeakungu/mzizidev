...
class PayoutCalculator:
    ...
    @staticmethod
    def calculate_dice_payout(bet_type: str, dice_roll: int, stake: Decimal) -> Tuple[bool, Decimal]:
        """
        Dice game: predict high(>3)/low(<=3)/even/odd on a 1-6 roll.
        Fixed 1.92x payout on a win (matches the odds shown in the UI).
        Returns (won, payout).
        """
        if bet_type == "high":
            won = dice_roll > 3
        ...
        payout = (stake * Decimal("1.92")) if won else Decimal("0.00")
        return won, payout

    RED_NUMBERS = {1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36}

    @classmethod
    def calculate_roulette_payout(cls, winning_number: int, bets: Dict[str, Any], stake_unit: Decimal = None) -> Decimal:
        """
        European roulette payout, computed from the server-determined
        winning number — never from a client-submitted number.

        `bets` maps bet key -> staked amount, e.g.
        {"17": 50, "red": 20, "even": 10}. Straight bets pay 36x their
        stake (35:1 + original), outside bets (red/black/even/odd) pay 2x.
        """
        total_payout = Decimal("0.00")
        is_red = winning_number in cls.RED_NUMBERS
        is_black = winning_number != 0 and not is_red

        for bet_key, amount in (bets or {}).items():
            amount = Decimal(str(amount))
            if bet_key.isdigit() and int(bet_key) == winning_number:
                total_payout += amount * Decimal("36")
            elif bet_key == "red" and is_red:
                total_payout += amount * Decimal("2")
            elif bet_key == "black" and is_black:
                total_payout += amount * Decimal("2")
            elif bet_key == "even" and winning_number != 0 and winning_number % 2 == 0:
                total_payout += amount * Decimal("2")
            elif bet_key == "odd" and winning_number % 2 == 1:
                total_payout += amount * Decimal("2")

        return total_payout

    # Sparse multiplier table matching the client UI display (app/templates/games/mines.html).
    # Keyed by tiles_revealed -> [difficulty 1, 2, 3] multiplier. Values between the
    # defined tile counts intentionally fall back to 1.0x, matching existing client behavior.
    MINES_MULTIPLIERS = {
        1: [1.06, 1.11, 1.23],
        5: [1.35, 1.59, 2.46],
        10: [1.82, 2.46, 6.17],
    ...
    @classmethod
    def calculate_mines_multiplier(cls, tiles_revealed: int, difficulty: int) -> Decimal:
        """Ceiling multiplier for a claimed tiles_revealed count. Interim
        validation only — see settle_round's mines branch for why this
        can't fully replace server-side path tracking yet."""
        mults = cls.MINES_MULTIPLIERS.get(tiles_revealed)
        if not mults or not (1 <= difficulty <= len(mults)):
            return Decimal("1.0")
        return Decimal(str(mults[difficulty - 1]))
