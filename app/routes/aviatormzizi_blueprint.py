...
def api_stats():
    ...
    try:
        # crash_stats is a relationship - get the first (or only) record
        stats_list = getattr(current_user, 'crash_stats', None)
        if not stats_list or len(stats_list) == 0:
            stats = CrashStats(user_id=current_user.id)
            db.session.add(stats)
            db.session.commit()
        else:
            # If it's a list/relationship, get the first one
