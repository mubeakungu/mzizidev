...
event names/shapes without rebuilding the Rea"""
AviatorCrash — real backend for the Unity WebGL "aviator-crash-main" game.
...
# BUILD_DIR is now relative to project root (not app/routes/)
...
BUILD_DIR = os.path.join(_root_dir, "static", "aviatorcrash")
...
def _make_game_blueprint(name, url_prefix):
    ...
    @login_required
    def index():
        # BUILD_DIR is now an absolute path, use it directly
...
def get_aviatorcrash_blueprints(sio, app):
    ...
    return aviator_bp, jetx_bp, api_bpct app):
