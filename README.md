...
cp .env.example .env   # fill in your real values (SQLite is used automatically if DATABASE_URL is unset)
python run.py           # tables are created and the catalog auto-seeded on first boot
```

The app boots on `http://localhost:8000` by default (`PORT` env var to change it).
Game catalog seeding (`seed_fixed.py`) and DB table creation both run
automatically inside `create_app()` — no separate migrate/seed step is
needed for local dev. Use `flask db migrate` / `flask db upgrade` only
once you're managing schema changes against a real Postgres database.
